...
namespace Map
{
    public static class MapGeneratorSO
    {
        private static MapConfigSO config;

        private static readonly List<NodeType>
            RandomNodes =
                new List<NodeType> {
                    NodeType.RestSite,
                    NodeType.Store,
        ...
        public static Map GetMap(MapConfigSO conf)
        {
            if (conf == null)
            {
            ...
            var bossNodeName =
                config
                    .nodeTemplates
                    .Where(b => b.nodeType == NodeType.Boss)
                    .ToList()
                    .Random()
                    .name;
            return new Map(conf.name,
                bossNodeName,
                nodesList,
                new List<Point>(),
                layerDistances);
        ...
        private static void PlaceLayer(int layerIndex)
        {
            ...
            {
                var nodeType =
                    Random.Range(0f, 1f) < layer.randomizeNodes
                        ? GetRandomNode()
                        : layer.nodeType;
                var templateName =
                    config
                ...
                var node =
                    new Node(nodeType,
                        templateName,
                        new Point(i, layerIndex))
                    {
                        position =
                            new Vector2(-offset + i * layer.nodesApartDistance,
        ...
        private static void RandomizeNodePositions()
        {
            for (var index = 0; index < nodes.Count; index++)
            {
                ...
                foreach (var node in list)
                {
                    ...
                    var y =
                        yRnd < 0
                            ? distToPreviousLayer * yRnd / 2f
                            : distToNextLayer * yRnd / 2f;

                    node.position +=
                        new Vector2(x, y) * layer.randomizePosition;
                }
            }
        }
        ...
        private static void RemoveCrossConnections()
        {
            ...
            for (var j = 0; j < config.layers.Count - 1; ++j)
            {
                ...
                if (
                    !node
                        .outgoing
                        .Any(element => element.Equals(topRight.point))
                ) continue;
        ...
        private static void GeneratePaths()
        {
            ...
            for (var i = 0; i < config.GridWidth; i++) candidateXs.Add(i);

            candidateXs.Shuffle();
            var startingXs = candidateXs.Take(numOfStartingNodes);
            var startingPoints =
                (from x in startingXs select new Point(x, 0)).ToList();

            candidateXs.Shuffle();
            var preBossXs = candidateXs.Take(numOfPreBossNodes);
            var preBossPoints =
                (from x in preBossXs select new Point(x, finalNode.y - 1))
                    .ToList();

            int numOfPaths =
                Mathf.Max(numOfStartingNodes, numOfPreBossNodes) +
                Mathf.Max(0, config.extraPaths);
